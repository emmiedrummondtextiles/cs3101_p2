...
-- 3. serviceEDBDEE: stops (incl. origin & destination) for the 18:59 1L27 service
...
SELECT
  ...
  st_i.code                         AS stn,
  sp.pl                             AS pl,
  CONCAT(LPAD(p.adh, 2, '0'), LPAD(p.adm, 2, '0')) AS arr,
  CONCAT(LPAD(p.ddh, 2, '0'), LPAD(p.ddm, 2, '0')) AS dep
FROM plan p
JOIN stop sp ON sp.hc = p.hc AND sp.frm = p.frm AND sp.loc = p.loc
JOIN service s ON p.hc = s.hc AND s.hc = '1L27' AND s.dh = '18' AND s.dm = '59'
JOIN station st_i ON p.loc = st_i.loc

UNION ALL

-- destination row (infinite departure diff)
SELECT
  p.loc                             AS loc,
  st_d.code                         AS stn,
  NULL                              AS pl,
